...
public class Game extends Observable {
	private static int[][] solution; // Generated solution.
	private static int[][] game; // Generated game with user input.
	...
	public static void main(String[] args) {
		Game gam = new Game();
		solution = gam.generateSolution(new int[9][9], 0);
		game = gam.generateGame(gam.copy(solution));
		for(int i=0;i<9; i++) {
	...
	private boolean isPossible(int[][] game, int x, int y, int number) {
		return isPossibleX(game, y, number) && isPossibleY(game, x, number) && isPossibleBlock(game, x, y, number);
	}
	...
	private boolean isValid(int[][] game, int index, int[] numberOfSolutions) {
		...
		if (game[y][x] == 0) {
			...
			return isValid(game, index + 1, numberOfSolutions);
		}
